...
INPUT=../../leblancx4-16node.pnt
EXEC=../../pennant.APPEKG
EXECHPAS=/anvil/projects/x-cis230165/tools/HPAS/install/bin/hpas

echo "pennant.APPEKG MEMBW jobID $SLURM_JOB_ID"
pwd
echo -e "srun --overlap --ntasks-per-node 8 --cpu-bind=map_cpu:0,4,8,16,64,68,72,76 ${EXECHPAS} membw -s 10K -t 0.0 -d -1.0 & \n"
echo -e "time srun --overlap --ntasks-per-node 8 -c 16 --cpu-bind=cores ${EXEC} ${INPUT} \n"

srun --overlap --ntasks-per-node 8 --cpu-bind=map_cpu:0,4,8,16,64,68,72,76 ${EXECHPAS} membw -s 10K -t 0.0 -d -1.0 &
